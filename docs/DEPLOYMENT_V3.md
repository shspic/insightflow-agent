...
> 并已在独立临时环境演练验证。
>
> 当前项目**未**部署到任何公网平台，也未声称已部署；未经授权不创建云资源、
> 不发布公网服务。真实备案号、真实运营主体信息、真实 API Key 一律不进入本仓库。
...
- 未在任何真实云平台部署（腾讯云/阿里云等）——本手册为操作说明，
  演练在隔离 Docker 环境完成；
- 未购买域名、未办理 ICP/公安备案、未申请真实证书；
- certbot 真实签发与续期、云监控/告警渠道、真实 BGE 模型下载与上传未在公网验证；
- 多用户高频并发下的 SQLite 承载未验证（当前按单用户/低频演示设计）。
